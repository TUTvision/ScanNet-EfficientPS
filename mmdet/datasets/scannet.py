...
PALETTE = [[174, 199, 232], [152, 223, 138], [31, 119, 180],  [255, 187, 120],
           ...
           [247, 182, 210], [219, 219, 141], [255, 127, 149], [158, 218, 229],
...
class ScanNetDataset(CocoDataset):
    ...
    CLASSES = ("cabinet",  "bed",     "chair",        "sofa",    "table",
    ...
    def _evaluate_panoptic(self, results, txtfile_prefix, logger):
        ...
        for pred_ann in os.listdir(pred_json):
            ...
            for el in tmp_json['annotations']:
                ...
                pred_annotations.update({image_id: el})

                # raiviol: quick fix for image size mismatch in evaluation

                orig_size = (1296, 968)
                path = Path(pred_folder) / Path(el['file_name'])

                bgr = cv2.imread(str(path), cv2.IMREAD_UNCHANGED)

                bgr = cv2.resize(bgr, orig_size, interpolation = cv2.INTER_NEAREST)
        ...
        for gt_ann in gt_json['annotations']:
            ...
            if image_id not in pred_annotations:
                if image_id.split('.')[0] in pred_annotations:
                    image_id = image_id.split('.')[0]
                    gt_ann['image_id'] = image_id
                else:
